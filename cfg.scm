...
(define-record cfg-node (succs preds))

(define-record cfg-state-line (label number))

;; block -> set(label)
(define cfg-get-succ-labels
  (lambda (block)
    (variant-case (block->jump block)
      (goto (label)               (singleton-set label))
      (return ()                  empty-set)
      (if (then-label else-label) (union-set (singleton-set then-label)
					     (singleton-set else-label)))
      (else (error "Unrecognized jump in cfg-get-succ-labels: "
		   (block->jump block))))))
...
(define cfg-state-first-in-block
  (lambda (block)
    (make-cfg-state-line (block->label block) 1)))

(define cfg-state-last-in-block
  (lambda (block)
    (let ((num-assigns (length (block->assigns block))))
      (make-cfg-state-line (block->label block)
			   (+ 1 num-assigns)))))
...
(define cfg-state-build-for-assigns
  (lambda (number preds cfg-state label length-assigns)
	...
	cfg-state
	(let* ((line      (make-cfg-state-line
			    label number))
	       (next-line (make-cfg-state-line
			    label
			    (+ number 1)))
	       (new-cfg-state
		 (update-table line
	   ...
	   (jump-pred-states (if (equal? length-assigns 0)
				 ...
				 (singleton-set
				   (make-cfg-state-line
				     label
				     length-assigns))))
	   (jump-line      (make-cfg-state-line label
						(+ 1 length-assigns))))
      (update-table jump-line
		    (make-cfg-node succ-states
				   jump-pred-states)
		    new-cfg-state))))
